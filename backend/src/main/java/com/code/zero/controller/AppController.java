...
public class AppController {
     ...
     * @return 生成结果流
     */
    @Operation(summary = "应用聊天生成代码", description = "应用聊天生成代码，需要应用权限")
    @GetMapping(value = "/chat/gen/code", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<ServerSentEvent<String>> chatToGenCode(@Parameter(description = "应用ID", required = true, example = "1") @RequestParam Long appId,
                                                       @Parameter(description = "用户消息", required = true, example = "我需要一个简单的任务记录工具网站") @RequestParam String message,
                                                       HttpServletRequest request) {
        // 参数校验
